...
class NewSignupPageSecondaryAuthenticationProvider extends AbstractSecondaryAuthenticationProvider {
	...
	public function testForAccountCreation( $user, $creator, array $reqs ) {
		...
		if ( $creator->isAllowed( 'bypasstoscheck' ) ) {
			return StatusValue::newGood();
		}

		if ( !$req || !$req->wpTermsOfService ) {
			return StatusValue::newFatal( 'newsignuppage-must-accept-tos' );
		}

		if ( !$req->wpAgeHonesty ) {
			return StatusValue::newFatal( 'newsignuppage-must-confirm-age' );
		}

		if ( NewSignupPageAuthenticationRequest::normalizeBirthDateValue( $req->wpBirthDate ) === null ) {
			return StatusValue::newFatal( 'newsignuppage-invalid-birthdate' );
		}
	...
	public function beginSecondaryAccountCreation( $user, $creator, array $reqs ) {
		...
		if ( !$req instanceof NewSignupPageAuthenticationRequest ) {
			return AuthenticationResponse::newPass();
		}
		$birthDate = NewSignupPageAuthenticationRequest::normalizeBirthDateValue( $req->wpBirthDate );
		if ( $birthDate !== null ) {
			$services->getUserOptionsManager()->setOption( $user, 'nsfw_birthdate', $birthDate );
			$services->getUserOptionsManager()->saveOptions( $user );
		}

		if (
			$birthDate !== null
			&& NewSignupPageAuthenticationRequest::isBirthDateAtLeastMinimumAge( $birthDate )
			&& class_exists( \ContinuumUniverses\ContinuumNsfwFilter\Hooks::class )
		) {
